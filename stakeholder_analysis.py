...
# Set path to either sentiment or fishery stance for stakeholder analysis
data_path = r'Data\fishery_lemmas_sentences_labeled_fishery_stance.csv'
#data_path = r'Data\fishery_lemmas_sentences_labeled_sentiment.csv'

# Paths to stakeholder lists
...
# Path to party data
party_data_path = r'Data\party_data.xlsx'

start_year = 2009
end_year = 2022

# Define political parties for filtering
party_names = {
    'SPD': {
        'names': ['Claus Tantzen', 'Till Backhaus', 'Barbara Hendricks', 
                  'Jochen Flasbarth', 'Bettina Hagedorn', 'Sönke Rix',
                  'Ulrike Rodust'],
    ...
    'CDU': {
        'names': ['Petra Nicolaisen', 'Julia Klöckner', 'Angela Merkel', 
                  'Daniel Günther', 'Juliane Rumpf', 'Peter Harry Carstensen', 
                  'Ingo Gädechens', 'Johannes Callsen', 'Philipp Amthor', 
                  'Robert Kloos', 'Werner Kuhn','Christian Schmidt'],
    ...
    'GREENS': {
        'names': ['Silke Schneider', 'Rasmus Andresen', 'Robert Habeck',
                  'Jan Philipp Albrecht', 'Steffi Lemke', 'Svenja Schulze',
                  'Friedrich Ostendorff'],
        
        'orgs': ['Bündnis 90 / Die Grünen', 'grünen', 'grüne', 'bündnis', 'grün']
    }
}
...
def analyze_by_party(common_pers, common_orgs, party_names, party_name, data, start_year, end_year, quota_dates):  
    
    if party_name == 'all_parties':
        
        stakeholders_pers_list = [name.lower() for party in party_names.values() for name in party['names']]
        stakeholders_orgs_list = [name for party in party_names.values() for name in party['orgs']]
        
    else:
    
        stakeholders_pers_list = [name.lower() for name in party_names[party_name]['names']]
        stakeholders_orgs_list = [name for name in party_names[party_name]['orgs']]
...
def write_summary_to_excel(summary_stats, filename, groups):
    wb = Workbook()
    ws = wb.active
...
def main(data_path, pers_path, orgs_path, quota_path, start_year, end_year, clean_PPA = True, party_data_path = ''):
    ...
    # Filter out all names and organistions related to Publics and Publics Authorities
    if clean_PPA == True:
        names_to_filter = []
        for party, info in party_names.items():
            names_to_filter.extend(info['names'])
        
        names_to_filter = [name.lower() for name in names_to_filter]
        
        common_pers = common_pers[~common_pers['Name'].isin(names_to_filter)]
        
        orgs_to_filter = []
        for party, info in party_names.items():
            orgs_to_filter.extend(info['orgs'])
        
        common_orgs = common_orgs[~common_orgs['Name'].isin(orgs_to_filter)] 
    ...
    analysis_results_stakeholders = {group: analyze_by_group(common_pers, common_orgs, group, data, start_year, end_year, quota_dates) for group in groups}
    
    # Further analysis based on political party
    party_list = ['CDU', 'SPD', 'GREENS', 'all_parties']
        
    analysis_results_parties = {party: analyze_by_party(common_pers, common_orgs, party_names, party, data, start_year, end_year, quota_dates) for party in party_list}
     
    def save_results(results,analysis_results, path, groups):
        ...
        for group, result in analysis_results.items():
            ...
            # First 2 years no observations for green party
            if group == 'GREENS':

                result_list = [df.iloc[2:] for df in result_list[0:6]]
                
            # 2011 years no observations for SPD
            elif group == 'SPD':

                result_list = result_list = [
                    pd.concat([df.iloc[:2], df.iloc[3:]])
                        for df in result_list[0:6]
                    ]
        ...
        write_summary_to_excel(summary_stats,f'Results_new\{path}\Summary_Statistics_tabS8_S9_S10.xlsx', groups)
        
        if groups ==  ['politics|management', 'science', 'fisheries', 'engo']:
        
            create_summary_excel_2(summary_stats,f'Results_new\{path}\Stakeholder_counts_tab1_tabS6.xlsx')

    save_results(results, analysis_results_stakeholders, 'all_stakeholders', ['politics|management', 'science', 'fisheries', 'engo'])
    save_results(results, analysis_results_parties, 'parties', ['CDU', 'SPD', 'GREENS', 'all_parties'])

    return results, analysis_results_stakeholders, analysis_results_parties, quota_dates, summary_stats

if __name__ == "__main__":
    results, analysis_results_stakeholders, analysis_results_parties, quota_dates, summary_stats = main(data_path, pers_path, orgs_path, quota_path, start_year, end_year)
        
    stakeholders_both_sum_pm = analysis_results_stakeholders['politics|management'][4]
    stakeholders_both_sum_f = analysis_results_stakeholders['fisheries'][4]
    stakeholders_both_sum_s = analysis_results_stakeholders['science'][4]
    stakeholders_both_sum_e = analysis_results_stakeholders['engo'][4]

    stakeholders_sentiment_both_pm = analysis_results_stakeholders['politics|management'][5].reset_index(drop=True)
    stakeholders_sentiment_both_f = analysis_results_stakeholders['fisheries'][5].reset_index(drop=True)
    stakeholders_sentiment_both_s = analysis_results_stakeholders['science'][5].reset_index(drop=True)
    stakeholders_sentiment_both_e = analysis_results_stakeholders['engo'][5].reset_index(drop=True)
    
    stakeholders_pers_sum_pm = analysis_results_stakeholders['politics|management'][0]
    stakeholders_pers_sum_f = analysis_results_stakeholders['fisheries'][0]
    stakeholders_pers_sum_s = analysis_results_stakeholders['science'][0]
    stakeholders_pers_sum_e = analysis_results_stakeholders['engo'][0]

    stakeholders_sentiment_pers_pm = analysis_results_stakeholders['politics|management'][1].reset_index(drop=True)
    stakeholders_sentiment_pers_f = analysis_results_stakeholders['fisheries'][1].reset_index(drop=True)
    stakeholders_sentiment_pers_s = analysis_results_stakeholders['science'][1].reset_index(drop=True)
    stakeholders_sentiment_pers_e = analysis_results_stakeholders['engo'][1].reset_index(drop=True)
    
    stakeholders_orgs_sum_pm = analysis_results_stakeholders['politics|management'][2]
    stakeholders_orgs_sum_f = analysis_results_stakeholders['fisheries'][2]
    stakeholders_orgs_sum_s = analysis_results_stakeholders['science'][2]
    stakeholders_orgs_sum_e = analysis_results_stakeholders['engo'][2]

    stakeholders_sentiment_orgs_pm = analysis_results_stakeholders['politics|management'][3].reset_index(drop=True)
    stakeholders_sentiment_orgs_f = analysis_results_stakeholders['fisheries'][3].reset_index(drop=True)
    stakeholders_sentiment_orgs_s = analysis_results_stakeholders['science'][3].reset_index(drop=True)
    stakeholders_sentiment_orgs_e = analysis_results_stakeholders['engo'][3].reset_index(drop=True)
...
def save_object(obj, name, dir='saved_objects'):
    path = os.path.join(dir, f"{name}.pkl")
    with open(path, 'wb') as f:
        pickle.dump(obj, f)
        
save_object(analysis_results_stakeholders)
save_object(analysis_results_parties)
...
def fishery_year(data, quota_dates):
    ...
    for idx, date in enumerate(quota_dates[:-1]):
      ...
      #  else:
          ...
          temp_df = data.loc[mask].copy()
    
          # Extract the year from the start_date and add it as a new column
          temp_df["fishery_year"] = start_date.year
          
          fishery_years.append(temp_df)
         # fishery_years.append(data[(data["Date"] >= start_date) & (data["Date"] < end_date)])
...
def sentiment_index_yearly(data): 
    ...
    if data != []:
        ...
        years = list(set(pd.concat(data)['fishery_year'])) 
        years.sort() 

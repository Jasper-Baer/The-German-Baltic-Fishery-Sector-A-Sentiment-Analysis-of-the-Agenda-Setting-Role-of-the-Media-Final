...
rolling_sent = sentiment_all_quarterly['Sentiment Index'].rolling(window=3).mean()[4:-1]
dates = sentiment_all_quarterly['Date'][4:-1]

# Plot the rolling mean of the sentiment index
ax1.plot(dates, rolling_sent, color='black', label='Sentiment Index')
...
quarter_indices = np.arange(1, len(dates) + 1)

# Perform linear regression on the sentiment
#slope, intercept, r_value, p_value, std_err = linregress(quarter_indices, sentiment_all_quarterly['Sentiment Index'][4:-1])
#print("Slope: {:.6f}, Standard Error: {:.6f}".format(slope, std_err))

# Create trend line data using the regression results
#trend_line = intercept + slope * quarter_indices

# Plot the linear trend line
#ax1.plot(dates, trend_line, color='red', linestyle='--', linewidth=2, label='Linear Trend')
...
plt.savefig(r'Figures\figure2.eps', format='eps', bbox_inches='tight')
plt.show()
...
quarter_indices = np.arange(1, len(sentiment_cod_quarterly['Date'][1:-1])+1)

# Perform linear regression on the sentiment
slope, intercept, r_value, p_value, std_err = linregress(quarter_indices, sentiment_cod_quarterly['Sentiment Index'][1:-1])
print("Slope: {:.6f}, Standard Error: {:.6f}".format(slope, std_err))
...
# Plot the linear trend line
ax2.plot(sentiment_cod_quarterly['Date'][1:-1], trend_line, color='red', linestyle='--', linewidth=2, label='Linear Trend')
...
quarter_indices = np.arange(1, len(sentiment_herring_quarterly['Date'][1:-1])+1)

# Perform linear regression on the sentiment
slope, intercept, r_value, p_value, std_err = linregress(quarter_indices, sentiment_herring_quarterly['Sentiment Index'][1:-1])
print("Slope: {:.6f}, Standard Error: {:.6f}".format(slope, std_err))
...
# Plot the linear trend line
ax4.plot(sentiment_herring_quarterly['Date'][1:-1], trend_line, color='red', linestyle='--', linewidth=2, label='Linear Trend')
